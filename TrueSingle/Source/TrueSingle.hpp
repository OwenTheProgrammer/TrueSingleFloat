...
union RegisterHalf16 {
	uint16_t Register;
	uint8_t Half[2];
};
union RegisterHalf64 {
	uint64_t Register;
	uint32_t Half[2];
};
union FloatBinding {
...
//Exports
DLL_EXPORT void Encode(Float8* Instance, const float Value) {
...
DLL_EXPORT Float8* CreateFloat8(const float Value, const uint8_t ExpBits) {
	...
	if(!ExpBits || ExpBits>5)
		return new Float8{0u, UINT8_MAX};
...
DLL_EXPORT const float ToSingle(Float8* Instance) {
...
DLL_EXPORT const void ChangeExponent(Float8* Instance, const uint8_t NewExp) {
	Float8& inst = *Instance;
...
DLL_EXPORT const float GetSerialError(Float8* Instance, const float v) {
...
DLL_EXPORT const uint8_t GetMemory(Float8* Instance) {
	return Instance->Memory;
}
DLL_EXPORT const uint8_t GetExponent(Float8* Instance) {
